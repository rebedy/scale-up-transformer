...
def eval_decorator(fn):
    def inner(model, *args, **kwargs):  # args: (text[:1]), kwargs: {'filter_thres': 0.9}
        ...
        model.eval()
        out = fn(model, *args, **kwargs) # generated image: [b, 3, 256, 256]
...
@torch.no_grad()
def generate(model,
             # TODO: projection matrix 고정시키기 -> before DDP 
    ...
    for cur_len in range(image_seq_len+1, max_seq_len):
...
#         if num_beams <= 1:
#             for cur_len in range(img_seq_len+1, max_seq_len):
...
#             generated_hyps = [BeamHypotheses(num_beams, max_seq_len, length_penalty, early_stopping=early_stopping)
#             for _ in range(b)]   
