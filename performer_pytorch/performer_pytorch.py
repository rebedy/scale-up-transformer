...
def softmax_kernel(data, *, projection_matrix, is_query, normalize_data=True, eps=1e-4, device = None): # projection_matrix: [nb_features, dim_heads]. chunk마다는 orthogonal하지만 다른 chunk의 vector와는 not orthogonal.
    ...
    data_normalizer = (data.shape[-1] ** -0.25) if normalize_data else 1.  # NOTE: 왜 ** -0.25이지? ** -0.5가 아니라?  왜 1/sqrt(d)가 아니냐. 그 이유는, query쪽에서 한 번, key쪽에서 한 번 해줘서 둘을 곱하면 ** -0.5가 됨. 
...
def conditioned_causal_linear_attention_noncuda(q, k, v, condition_len, chunk_size =1, eps = 1e-6): # q, k: [B, global_head, seq_len, nb_features]  v: [B, global_head, seq_len, dim_head]
    ...
    condition_k_sum = condition_k.sum(dim = -2) # -> [B, global_head, nb_features]  논문의 식에서 (K')T 1L 에 해당. 
    condition_D_inv = 1. / torch.einsum('...nd,...d->...n', condition_q, condition_k_sum.type_as(condition_q))  # [B, global_head, seq_len] 논문의 식에서 Q' ((K')T 1L)에 해당.
    ...
    last_k_cumsum = condition_k_sum.unsqueeze(2)
...
def conditioned_causal_linear_attention(q, k, v, condition_len, chunk_size = 32, eps = 1e-6): # q, k: [B, global_head, seq_len, nb_features]  v: [B, global_head, seq_len, dim_head]
    ...
    with cuda_context():
        ...
        out = causal_dot_product_fn(q, k, v)  # [B, head, seq_len, nb_features]
...
def conditioned_causal_linear_attention_cuda(q, k, v, condition_len, eps=1e-6):  # q, k: [B, global_head, seq_len, nb_features]  v: [B, global_head, seq_len, dim_head]
    ...
    condition_q2 = q[:, :, condition_len:] #q,k:[B, global_head, seq_len/chunk_size, nb_features]
    condition_k2 = k[:, :, condition_len:]
    condition_v2 = v[:, :, condition_len:] #  v:[B, global_head, seq_len/chunk_size, dim_head]
    
    with cuda_context():
        if autocast_enabled:
            condition_q2, condition_k2, condition_v2 = map(lambda t: t.float(), (condition_q2, condition_k2, condition_v2))

        out = causal_dot_product_fn( condition_q2, condition_k2, condition_v2)  # -> [B, global_head, seq_len-condition_len, dim_head]

    # condition_context 
    condition_add_out = torch.einsum('...de,...nd->...ne', condition_context, condition_q2) # 3 사분면
...
class FastAttention(nn.Module):
    def __init__(self, dim_heads, nb_features = None, ortho_scaling = 0, 
                 causal = False, condition_len = 0, attn_type="conditioned_noncuda", 
                 generalized_attention = False, kernel_fn = nn.ReLU(), no_projection = False):
        super().__init__()
...
class Attention(nn.Module):
    def __init__(   ## TODO: 여기 들어가는 argument들 각자의 역할 주석 달기
        self,
        dim,   # 총 모델 안에 들어가는 dimension
        causal = False,
        condition_len = 0,
        ...
        nb_features = None,  # 논문에서의 r
        feature_redraw_interval = 1000,
        generalized_attention = False,  # 우리가 하고자 하는게 Softmax Kernel이면 False, 만약 다른 거면 True
        kernel_fn = nn.ReLU(),  # 만약 generalized_attention이 True라면 직접 우리가 커널을 정의
        dropout = 0.,
        no_projection = False,
        qkv_bias = False,
        attn_out_bias = True
        ...
        dim_head = default(dim_head, dim // heads)  # dim_head가 존재하면 그걸 그대로 return // 각 attention head마다의 dimension
        inner_dim = dim_head * heads  # 최종 encoder의 dim
        self.fast_attention = FastAttention(dim_head, nb_features, causal = causal, condition_len = condition_len, attn_type=attn_type, 
    ...
    def forward(self, x, pos_emb = None, context = None, mask = None, context_mask = None, **kwargs): # x: [B, seq_len, dim]  pos_emb: [1, seq_len, dim_head]  mask: [B, seq_len] 여기서 mask는 padding용
        b, n, _, h, gh = *x.shape, self.heads, self.global_heads   # eg. self.heads = 8, self.global_heads = 4

        cross_attend = exists(context)  # 보통 False 우리는 self-attention만 사용하니까
        ...
        if not empty(q): # 만약 global head가 존재한다면
        ...
        if not empty(lq):  # 만약 local attention이 있다면
...
class SelfAttention(Attention):  # 여기에서 __init__()을 정의하지 않은 것은, Attention에 있는 __init__()을 덮어쓰지 않고 그대로 쓰겠다는 것
    def forward(self, *args, context = None, **kwargs):
        assert not exists(context), 'self attention should not receive context'  # context란 cross attention에 있는 encoder의 ouput을 의미, 고로 self-attention에서는 context가 없어야 한다.
...
class Performer(nn.Module):
    def __init__(
        ...
        for _, local_heads in zip(range(depth), local_attn_heads):
            ## Self-Attention + Feed Forward 부분
            layers.append(nn.ModuleList([  # f, g
                wrapper_fn(SelfAttention(dim, causal = causal, condition_len = condition_len, attn_type=attn_type, 
                                         heads = heads, dim_head = dim_head, local_heads = local_heads, local_window_size = local_window_size, 
                                         nb_features = nb_features, generalized_attention = generalized_attention, kernel_fn = kernel_fn, 
                                         dropout = attn_dropout, no_projection = no_projection, qkv_bias = qkv_bias, attn_out_bias = attn_out_bias)),
...
class PerformerLM(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.performer = Performer(dim, depth, heads, dim_head, local_attn_heads, local_window_size, causal, condition_len, attn_type, 
                                   ff_mult, nb_features, feature_redraw_interval, reversible, ff_chunks, generalized_attention, kernel_fn, 
                                   use_scalenorm, use_rezero, ff_glu, ff_dropout, attn_dropout, cross_attend, no_projection, auto_check_redraw, 
