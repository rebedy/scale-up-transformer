...
#!#  
"""
CUDA_VISIBLE_DEVICES=0 python3 -m torch.distributed.launch --nproc_per_node=1 obw_main_trans.py --kernel_type favor --mpi_port 12567
"""
...
from loader_obw import DatasetForMLM
from helpers import *
from train_helper import *
import obw_parser as parser_
...
# from transformer_pytorch.transformer_mlm import TransformerLM
...
def build_dataloaders(dataset, batch_size, train_test_split=0.1, train_shuffle=False, eval_shuffle=False):
...
def main_worker(args: argparse.Namespace):
    ...
    #!# Dataset settings    
    print("\n 1-Billion-Words Dataset Loading...")
    #!# Tokenizer
    # tokenizer = BertTokenizer(vocab_file=config.vocab_path, do_lower_case=False)
    tokenizer = BertTokenizer.from_pretrained("bert-base-cased")
    args.num_tokens = tokenizer.vocab_size
    print("args.num_tokens ", args.num_tokens)
    vocab = tokenizer.get_vocab()
    
    dataset = DatasetForMLM(tokenizer, args.max_seq_len, path=os.path.join(args.dataset_dir, args.data_path))
    
    ## dataloader
    train_dl, train_sampler, val_dl, val_sampler = build_dataloaders(dataset, args.batch_size, train_test_split=0.1)



    if gpu == 0:
        if args.wandb:
            wandb.init(dir=args.save_dir_base, config = args, project="SUT_1BW_"+TODAY)
        print("\n")
        ...
        ## Make DIRs
        exp_name = f"sut_{TODAY}{NOW}_trans_{args.kernel_type}_txt{args.max_seq_len}_h{args.heads}_l{args.depth}_dff{args.d_ff}_d{args.dim}_bz{args.batch_size}"
    ...
    print("\nStart Training.. Transformer ", args.kernel_type)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, batch in tqdm.tqdm(enumerate(train_dl), mininterval=10., leave=True, 
            ...
            # loss = loss.item()
            # acc = sum(sum(accuracy(labels, logits.transpose(1, 2))))
            ...
            if int(i+1) % args.validate_every == 0 and i != 0:
                model.eval()
                _val_avg_loss, _val_avg_ppl, _val_avg_acc = [], [], []
                with torch.no_grad():
                    for j, batch in tqdm.tqdm(enumerate(val_dl), mininterval=10., leave=True, 
                        ...
                        val_acc = sum(sum(accuracy(labels, val_logits.transpose(1, 2))))
                        val_acc = val_acc.item()
                        
                        if gpu == 0:
                            _val_avg_acc.append(val_acc)
                            _val_avg_loss.append(val_loss)
                            val_ppl = math.exp(val_loss)
                            _val_avg_ppl.append(val_ppl)
                            val_logits = val_logits.transpose(1, 2)
                            correct = (seq == torch.argmax(val_logits, 1))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(parents=[parser_.parser], description="sut on transformer")
