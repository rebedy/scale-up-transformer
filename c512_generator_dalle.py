...
from loader_cnn import CXRDataset
from transformer_dalle.transformer_i2t_cnn import *

import c256_parser_dalle as parser_
...
## inference args
parser.add_argument('--task', default='val', choices=['val', 'test','train'], type=str)
parser.add_argument('--infer_batch_size', default=132, type=int)
...
parser.add_argument('--filter_logits_fn', default='both', choices=['top_k', 'top_p', 'both'], type=str)
...
parser.add_argument("--gen_save", default="/home/dylee/__workspace/scaleup_transformer/metadata/", type=str, help="Where to save report generation result csv file.")
parser.add_argument('--prefix', default='c256', choices=['q256', 'q512', 'c256', 'c512'], type=str)

args = parser.parse_args()

#######!########
                #"/home/edlab/dylee/scaleup_transformer/transformer_dalle/"
args.infer_ckpt= args.save_dir_base+"sut_c512_20211012_02h19m_totim1_im1_txt256_fixed_bz20_ep50_lr5e-05_relu/epoch=000012.ckpt"
#######!########
...
for epoch in range(args.start_epoch, args.start_epoch+1):
    ...
    for batch_idx, batch in enumerate(tqdm.tqdm(dl)):
        ...
        with torch.no_grad():
            ...
            for cur_len in range(image_seq_len+1, args.max_seq_len):
                ...
                if args.filter_logits_fn == 'both':
                    filtered_logits = filter_logits_fn(logits, top_p=args.top_p, top_k=args.top_k)
                else:
                    filtered_logits = filter_logits_fn(logits, thres = args.filter_thres)
    ...
    for output in decoded_texts.values():
        ...
        reference = tokenizer_.encode(output['GT_text']).ids
        candidate = tokenizer_.encode(output['gen_text']).ids
    ...
    print(f'1-Gram BLEU: {bleu1_chencherry:.2f}')
    print(f'2-Gram BLEU: {bleu2_chencherry:.2f}')
    print(f'3-Gram BLEU: {bleu3_chencherry:.2f}')
    print(f'4-Gram BLEU: {bleu4_chencherry:.2f}')
    print("\nGeometric Mean N-Gram Score (Cumulative Score)")
    ...
    print(f'4-Gram Cumulative BLEU (nltk) : {bleu_cum1_chencherry:.2f}')
    print(f'4-Gram Cumulative BLEU (scipy): {bleu_cum2_chencherry:.2f}')
    print(f'4-Gram Cumulative BLEU (hand) : {bleu_cum3_chencherry:.2f}')
    print("################################################")
    ...
    csv_save=True
    if csv_save:
        file_path = os.path.join(args.gen_save, str("trans_dalle_"+args.prefix+args.infer_ckpt.split("/")[-1][:-5]+"_"+args.filter_logits_fn+"/"))
