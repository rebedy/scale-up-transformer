...
from medvill.pytorch_pretrained_bert.tokenization import BertTokenizer
from medvill.pytorch_pretrained_bert.model import BertForPreTrainingLossMask, BertForSeq2SeqDecoder
from medvill.pytorch_pretrained_bert.optimization import BertAdam, warmup_linear
from medvill.loader_utils import batch_list_to_batch_tensors
from medvill.data_loader import *
from medvill.data_parallel import DataParallelImbalance
...
"""
python3 mdvl_finetune.py --tasks report_generation --mask_prob 0.15 --s2s_prob 1 --bi_prob 0
"""
...
def main():
    ...
    parser.add_argument("--generation_dataset", default='mimic-cxr', type=str,
    ...
    parser.add_argument("--train_batch_size",default=3,type=int,help="Total batch size for training.")
    ...
    # parser.add_argument("--config_path", default='/home/edlab/jhmoon/work_Multi-modality-Self-supervision/generation/pretrained_model/non_cross/config.json', type=str,
    ...
    if args.tasks=='vqa':
        ...
        if args.generation_dataset == 'mimic-cxr':
            ...
            wandb.config["more"] = "custom"
            args.src_file = 'metadata/mimiccxr_train_sub_with_count.csv'
            args.file_valid_jpgs = 'metadata/mimiccxr_train_sub_with_count.csv'
    ...
    tokenizer2.add_special_tokens(["[PAD]", "[SOS]", "[EOS]", "[SEP]", "[MASK]"])
    tokenizer2._tokenizer.post_processor = BertProcessing(
        ("[EOS]", tokenizer2.token_to_id("[EOS]")),
        ("[SOS]", tokenizer2.token_to_id("[SOS]")),
    )
    tokenizer2.enable_truncation(max_length=args.max_text_len)  # max_length: [SOS]와 [EOS]를 합친 최종길이의 최대값
    tokenizer2.enable_padding(pad_id=tokenizer2.token_to_id("[PAD]"), pad_token="[PAD]", max_length=args.max_text_len)  
                                                # 먼저 enable_truncation에 의해 자른 후 뒤를 length까지 [PAD]로 채운다
    vocab = tokenizer2.get_vocab()  #list(tokenizer.vocab.keys())
    # convert_tokens_to_ids = tokenizer.convert_tokens_to_ids
    convert_tokens_to_ids = tokenizer.encode
    #!#
    
    if args.do_train:
        print("args.mask_prob",args.mask_prob)
        print("args.train_batch_size",args.train_batch_size)
        bi_uni_pipeline = [Preprocess4Seq2seq(args, args.max_pred, args.mask_prob,
            vocab, convert_tokens_to_ids, args.max_seq_length, args.bar,
        ...
        bi_uni_pipeline.append(Preprocess4Seq2seq(args, args.max_pred, args.mask_prob,
            vocab, convert_tokens_to_ids, args.max_seq_length, args.bar,
            new_segment_ids=args.new_segment_ids, truncate_config={
            'max_len_b': args.max_len_b, 'trunc_seg': args.trunc_seg, 'always_truncate_tail':
            args.always_truncate_tail},
        ...
        train_dataset = Img2txtDataset(args, args.data_set,
            args.src_file, args.image_root, args.img_root_dir, args.text_root_dir,
            args.split, args.train_batch_size,
            tokenizer2, args.max_seq_length, file_valid_jpgs=args.file_valid_jpgs,
