...
class DatasetForMLM(Dataset):
    def __init__(self, tokenizer, max_len, path="/home/data_storage/dylee_135/one-billion-words/1-billion-word-language-modeling-benchmark/training-monolingual.tokenized.shuffled"):
        ...
        for file_name in file_list:#file_progress_bar:
          p = f'{path}/{file_name}'
          data_file = open(p, 'r+')#, encoding='utf-8')
    ...
    def mask_tokens(self, inputs: torch.Tensor, mlm_probability=0.15, pad=True): # mask token! 
    ...
    def __getitem__(self, idx):
      ...
      return inputs, inputs_mask, labels #inputmask.unsqueeze(0)
