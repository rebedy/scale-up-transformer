...
def cycle():
    while True:
        ...
        src = torch.randint(
            2, NUM_TOKENS, (BATCH_SIZE, ENC_SEQ_LEN)).long().cuda()
...
model, optim_ = amp.initialize(model, optimizer, opt_level='O1')
...
for i in tqdm.tqdm(range(NUM_BATCHES), mininterval=10., desc='training'):
    ...
    with amp.scale_loss(loss, optim_) as scaled_loss:
    ...
    if i != 0 and i % GENERATE_EVERY == 0:
        ...
        sample = model.generate(
            src, start_tokens, ENC_SEQ_LEN, enc_mask=src_mask)
