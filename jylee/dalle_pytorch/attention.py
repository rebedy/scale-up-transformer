...
def apply_pos_emb(pos_emb, qkv):
    n = qkv[0].shape(-2)   # sequence length
    pos_emb = pos_emb[..., :n, :]
...
class Attention(nn.Module):
    ...
    def forward(self, x, mask=None, rotary_pos_emb=None):   # mask: padding mask
        ...
        dots = torch.einsum('b h i d, b h j d -> b h i j', q, k)    # attention map 구하는 과정
        mask_value = max_neg_value(dots)   # 현재 데이터 타입중에서 표현할 수 있는 가장 낮은 값

        if exists(mask):
            mask = rearrange(mask, 'b j -> b () () j')
            dots.masked_fill_(~mask, mask_value)

        if self.causal:  # 현재 performer에서 하고 있는 attention과는 다르게 text에도 attention이 causal하게 걸리고 있다
            i, j = dots.shape[-2:]  # i = query의 sequence length, j = key의 sequence length
...
# sparse attention with convolutional pattern, as mentioned in the blog post. customized kernel size and dilation
class SparseConvCausalAttention(nn.Module):
    def __init__(self, dim, seq_len, image_size=32, kernel_size=5, dilation=1, heads=8, dim_head=64, dropout=0., stable=False, **kwargs):
        ...
        self.heads = heads
        self.scale = dim_head ** -0.5
        self.image_size = image_size  # 한 행에 얼마나 열이 있는지, 예를 들면 32*32 면 image size는 32
    ...
    def forward(self, x, mask=None, rotary_pos_emb=None):
        ...
        # padding   ## TODO: padding을 왜하지?
        padding = seq_len - n + 1
        mask = default(mask, lambda: torch.ones(b, text_len, device=device).bool())

        x = F.pad(x, (0, 0, 0, padding), value=0)   # TODO: 이걸 왜 하지?
        mask = mask[:, :text_len]

        # derive queries / keys / value
        qkv = self.to_qkv(x).chunk(3, dim=-1)
        q, k, v = map(lambda t: rearrange(t, 'b n (h d) -> (b h) n d', h=h), qkv)
        ...
        k_img, v_img = map(lambda t: rearrange(t, 'b (h w) c -> b c h w', h=img_size), (k_img, v_img))   # conv에 넣는 input처럼 만들어줌
        ...
        i, j = dots_image.shape[-2:]   # i = query length, j = key length
        ...
        out_image_to_text = einsum('b i j, bi j d -> b i d', attn_image_to_text, v_text)
...
# 아마 row, column attention 구하는데 사용할 듯
class SparseAxialCausalAttention(nn.Module):
    ...
    def forward(self, x, mask=None, rotary_pos_emb=None):
        ...
        x = F.pad(x, (0, 0, 0, padding), value=0)  ## TODO: 이게 뭐하는 거지?
        mask = mask[:, :text_len]
