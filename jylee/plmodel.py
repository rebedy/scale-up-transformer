...
from performer_pytorch import PerformerLM_i2t, PerformerLM_t2i
...
class PerformerLightning_i2t(pl.LightningModule):
    def __init__(self, lr=5e-4, weight_decay=0.01, tokenizer=None, pad_token_idx=0, sos_token_idx=1, eos_token_idx=2, sav_dir='./', **kargs):
        super().__init__()
        self.kargs = kargs
...
class PerformerLightning_t2i(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = cross_entropy(logit, target)   # 여기서는 ignore index가 없다. 왜냐면 패딩이 없기 때문에
        self.log('train_loss', loss)
        return loss
