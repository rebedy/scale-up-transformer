...
from transformer_pytorch.attention import Attention, SparseAttention, SparseConvCausalAttention, SparseAxialCausalAttention
from transformer_pytorch.transformer_pytorch import *
from g_mlp_pytorch import gMLPBlock


# token shift classes

class PreShiftTokenDALLE(nn.Module):
...
class TransformerDALLE(nn.Module):
    def __init__(
        self,
        *,
        dim,
        ...
        for ind, sparse_attn, attn_type in zip(range(depth), sparse_layer, attn_type_layer):
            if attn_type == 'full':
                attn_class = partial(Attention, stable_softmax = stable_softmax)
            elif attn_type == 'sparse':
                attn_class = SparseAttention
            elif attn_type == 'axial_axial':
                attn_class = partial(SparseAxialCausalAttention, seq_len = seq_len, axis = 0, image_size = image_fmap_size, stable_softmax = stable_softmax)
            elif attn_type == 'axial_col':
                attn_class = partial(SparseAxialCausalAttention, seq_len = seq_len, axis = 1, image_size = image_fmap_size, stable_softmax = stable_softmax)
            elif attn_type == 'conv_like':
                attn_class = partial(SparseConvCausalAttention, seq_len = seq_len, image_size = image_fmap_size, stable_softmax = stable_softmax)
            elif attn_type == 'mlp':
            ...
            if shift_tokens:
                attn, ff = map(lambda t: PreShiftTokenDALLE(t, image_size = image_fmap_size, seq_len = seq_len), (attn, ff))
