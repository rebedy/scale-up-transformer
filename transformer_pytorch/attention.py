...
class Attention(nn.Module):
    def __init__(self, dim, causal = True, heads = 8, dim_head = 64, dropout = 0., stable_softmax = False):
        super().__init__()
    ...
    def forward(self, x, mask = None, pos_emb = None):
        ...
        softmax = torch.softmax if not self.stable_softmax else stable_softmax

        qkv = self.to_qkv(x).chunk(3, dim = -1)
        q, k, v = map(lambda t: rearrange(t, 'b n (h d) -> b h n d', h = h), qkv)

        if exists(pos_emb):
            q, k, v = apply_pos_emb(pos_emb, (q, k, v))
...
class SparseConvCausalAttention(nn.Module):
    def __init__(self, dim, seq_len, image_size = 32, kernel_size = 5, dilation = 1, heads = 8, dim_head = 64, dropout = 0., stable_softmax = False, **kwargs):
        super().__init__()
...
class SparseAxialCausalAttention(nn.Module):
    def __init__(self, dim, seq_len, image_size = 32, axis = 0, heads = 8, dim_head = 64, dropout = 0., stable_softmax = False, **kwargs):
        super().__init__()
