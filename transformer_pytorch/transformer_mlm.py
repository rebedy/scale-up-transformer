...
def self_attention(query, key, value, mask=None):
  ...
  if mask is not None:
    attention_score = attention_score.float().masked_fill(mask == 0, -1e10)
...
class FeedForward(nn.Module):
  ...
  def forward(self, x):
    return self.w_2(self.dropout(nn.GELU(self.w_1(x)))) # F.relu(self.w_1(x))))
...
class Encoder(nn.Module):
  def __init__(self, d_model, mult, head_num, dropout):
    ...
    self.feed_forward = FeedForward(d_model, mult=mult)
...
class EncoderAttnWise(nn.Module):
    def __init__(self, attn_type, d_model, mult, head_num, dropout, generalized_attention, no_projection, chunk_size):
        super(EncoderAttnWise,self).__init__()
        if attn_type == 'full':
            self.multi_head_attention = MultiHeadAttention(d_model= d_model, head_num= head_num)
        elif attn_type == 'perf':
          self.multi_head_attention = FAVORAttention(dim=d_model,
                                                     causal = None, 
                                                     heads = head_num, 
...
class Transformer(nn.Module):
  def __init__(self,vocab_num, d_model, ff_mult, max_seq_len, head_num, dropout, N):
    super(Transformer,self).__init__()
    self.embedding = Embeddings(vocab_num, d_model)
    self.positional_encoding = PositionalEncoding(max_seq_len,d_model)

    self.encoders = clones(Encoder(d_model=d_model, mult=ff_mult, head_num=head_num, dropout=dropout), N)
    self.decoders = clones(Decoder(d_model=d_model, mult=ff_mult, head_num=head_num, dropout=dropout), N)
...
#   def __init__(self, vocab_size, ff_mult, dim=512,  depth= 12, max_seq_len=512, head_num=8, dropout= 0.1):
#     super(TransformerLM,self).__init__()

#     self.token_emb= nn.Embedding(vocab_size, dim)
#     self.position_emb = PositionalEmbedding(dim,max_seq_len)
#     self.encoders = clones(Encoder(d_model=dim, mult=ff_mult, head_num=head_num, dropout=dropout), depth)
...
class TransformerLM(nn.Module):
    def __init__(self, 
        ...
        self.token_emb= nn.Embedding(vocab_size, dim)
        self.position_emb = PositionalEmbedding(dim,max_seq_len)  # ==AbsolutePositionalEmbedding
        self.encoders = clones(EncoderAttnWise(attn_type=attn_type, 
                                               mult=ff_mult,
                                               d_model=dim, 
                                               head_num=head_num, 
                                               dropout=dropout, 
        ...
        self.proj_updater = ProjectionUpdater(self.lm_head, feature_redraw_interval)
