...
#!#Full attention bidrectional 하게 모두 다 걸리는 attention
...
class ConditionedCausalAttention_i2t(nn.Module):
    def __init__(self, 
                 dim, 
                 seq_len, 
                 condition_len, 
    ...
    def forward(self, x, mask = None, rotary_pos_emb = None):
        ...
        qkv = self.to_qkv(x).chunk(3, dim = -1)
        q, k, v = map(lambda t: rearrange(t, 'b n (h d) -> b h n d', h = h), qkv) #[b, 12, 512, 64]
        ...
        #!# Image
        # torch.Size([b, 12, 256, 64])
        condition_q = q[:, :, :self.condition_len]  # [B, global_head, condition_len, nb_features] 
        condition_k = k[:, :, :self.condition_len]  # [B, global_head, condition_len, nb_features]
        condition_v = v[:, :, :self.condition_len]  # [B, global_head, condition_len, dim_head]
        
        img_dots = torch.einsum('b h i d, b h j d -> b h i j', condition_q, condition_k)
        mask_value = max_neg_value(img_dots)

        if exists(mask):
            mask = rearrange(mask, 'b j -> b () () j')
            img_dots.masked_fill_(~mask, mask_value)
            del mask
        
        img_attn = softmax(img_dots, dim=-1)  #img_attn.shape torch.Size([16, 12, 256, 256])
        '''
            q.shape torch.Size([16, 12, 512, 64])
            k.shape torch.Size([16, 12, 512, 64])
            v.shape torch.Size([16, 12, 512, 64])
            img_attn.shape torch.Size([16, 12, 256, 256])
            v.shape torch.Size([16, 12, 512, 64])
        '''
        img_out = torch.einsum('b h i j, b h j d -> b h i d', img_attn, condition_v)
        out_image = rearrange(img_out, 'b h n d -> b n (h d)')   

        
        # print(x.shape)
        # print(q.shape)
        #!# Text 
        q = q[:, :, self.condition_len:]  # [B, global_head, condition_len, nb_features]
        k = k[:, :, self.condition_len:]  # [B, global_head, condition_len, nb_features]
        v = v[:, :, self.condition_len:]  # [B, global_head, condition_len, dim_head]
        # print(q.shape)
        # torch.Size([20, 512, 768])
        # torch.Size([20, 12, 512, 64])
        # torch.Size([20, 12, 0, 64])
        ...
        attn = softmax(dots, dim=-1)
        out = torch.einsum('b h i j, b h j d -> b h i d', attn, v)
        out_text = rearrange(out, 'b h n d -> b n (h d)')
...
# customizable kernel size and dilation. (ETC같은건가...?)
...
###!### Custome by DY based on Sparse Axial Causal Attention
class ConditionedCausalAttention_i2t_(nn.Module):
    def __init__(self, 
                 dim, 
                 seq_len, 
                 condition_len, 
        ...
        self.heads = heads
        self.seq_len = seq_len
        self.condition_len=condition_len+1
        self.scale = dim_head ** -0.5
        self.image_size = image_size
    ...
    def forward(self, x, mask = None, rotary_pos_emb = None):
        b, n, _, h, img_size, axis, seq_len, device = *x.shape, self.heads, self.image_size, self.axis, self.seq_len, x.device
        softmax = torch.softmax if not self.stable else stable_softmax
        # print("x.shape : ", x.shape) #x.shape :  torch.Size([16, 512, 768])
        
        img_seq_len = img_size ** 2 #256
        # text_len = seq_len + 1 - img_seq_len #257
        text_len = seq_len - img_seq_len #256
        # print("img_seq_len : ", img_seq_len)
        ...
        ##!## Conditioned Q, K, V
        ((q_img,q_text), (k_img,k_text), (v_img,v_text)) =\
            map(lambda t: (t[:, :self.condition_len], t[:, self.condition_len:]), (q, k, v))
        # print(q_img.shape) k_img, v_img == torch.Size([192, 257, 64])


        ##!## TEXT Attention
        dots_text = einsum('b i d, b j d -> b i j', q_text, k_text)
        mask_value = max_neg_value(dots_text)  
        #-torch.finfo(t.dtype).max ; Machine limits for floating point types.
        #ex. np.finfo(np.float64).max = 1.7976931348623157e+308

        i, j = dots_text.shape[-2:]
        ## text를 위한 causal mask!
        text_causal_mask = torch.ones(i, j, device = device).triu_(j - i + 1).bool()
        dots_text.masked_fill_(text_causal_mask, mask_value)
        ...
        # # split out axis
        # q_img, k_img, v_img = map(lambda t: rearrange(t, split_axis_einops, h = img_size), (q_img, k_img, v_img))
        # # q_img, k_img, v_img = map(lambda t: rearrange(t, split_axis_einops, h = h), (q_img, k_img, v_img))
        ...
        # print("dots.shape : ", dots.shape) #dots.shape :  torch.Size([192, 256, 513])

        #!# MASKING so that text has full attention to image, but causal along axis
                #bafore: mask so image has full attention to text, but causal along axis
        # bh, x, i, j = dots.shape
        bh, i, j = dots.shape

        causal_mask = torch.ones(i, h, device = device).triu_(h - i + 1).bool()
        # print("causal_mask\n", causal_mask)

        # causal_mask = repeat(causal_mask, 'i j -> b x i j', b = bh, x = x)
        causal_mask = repeat(causal_mask, 'i j -> b i j', b = bh, x = x)
        # print("causal_mask.shape : ", causal_mask.shape)

        # mask = repeat(mask, 'b j -> (b h) x i j', h = h, x = x, i = i)
        mask = repeat(mask, 'b j -> (b h) i j', h = h, x = x, i = i)
        ...
        #!# Aggregate
        attn_text_to_image, attn_text_to_text = attn[..., :self.condition_len], attn[..., self.condition_len:]
        out_text_to_image = einsum('b i j, b j d -> b i d', attn_text_to_text, v_text)
        out_text_to_text = einsum( 'b i j, b x j d -> b i d', attn_text_to_image, v_img)
        out_image = attn_text_to_text + attn_text_to_image
        # print("out_image.shape : ", out_image.shape)
        
        # merge back axis
        # out_image = rearrange(out_image, merge_axis_einops, x = h)
