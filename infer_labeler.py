...
def get_label_accuracy_v3(target, reference):
    ...
    for row in range(len(df_tgt)):
        if len(positive_ref.loc[row].unique()) != 1:
            positive_precision = precision_score(positive_ref.loc[row],positive_tgt.loc[row], average="binary", pos_label=True, zero_division=0)
            positive_recall = recall_score(positive_ref.loc[row],positive_tgt.loc[row], average="binary", pos_label=True, zero_division=0)
            pos_precision.append(positive_precision)
            pos_recall.append(positive_recall)
        if len(negative_ref.loc[row].unique()) != 1:
            negative_precision = precision_score(negative_ref.loc[row],negative_tgt.loc[row], average="binary", pos_label=True, zero_division=0)
            negative_recall = recall_score(negative_ref.loc[row],negative_tgt.loc[row], average="binary", pos_label=True, zero_division=0)
            neg_precision.append(negative_precision)
            neg_recall.append(negative_recall)
        if len(ambi_ref.loc[row].unique()) != 1:
            ambi_precision = precision_score(ambi_ref.loc[row],ambi_tgt.loc[row], average="binary", pos_label=True, zero_division=0)
            ambi_recall = recall_score(ambi_ref.loc[row],ambi_tgt.loc[row], average="binary", pos_label=True, zero_division=0)
        ...
        all_precision = precision_score(df_ref.iloc[row,1:].fillna(0).values, df_tgt.iloc[row,1:].fillna(0).values, average='macro', zero_division=0)
        all_recall = recall_score(df_ref.iloc[row,1:].fillna(0).values, df_tgt.iloc[row,1:].fillna(0).values, average='macro', zero_division=0)
