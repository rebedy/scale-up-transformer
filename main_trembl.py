...
def main_worker(args: argparse.Namespace):
    ...
    gpu = args.gpu
    # int((8 + args.ngpus_per_node - 1) / args.ngpus_per_node)
    args.num_workers = 0
    ...
    train_sampler = distributed.DistributedSampler(
        train_ds, drop_last=True, seed=args.seed)
    train_dl = DataLoader(train_ds, batch_size=args.batch_size, pin_memory=True,
                          num_workers=args.num_workers, sampler=train_sampler)
    val_ds = TREMBLDataset(args.val_file, args.max_seq_len)
    val_sampler = distributed.DistributedSampler(
        val_ds, drop_last=True, seed=args.seed)
    ...
    if gpu == 0:
        ...
        # Make DIRs
        exp_name = f"sut_favor_{TODAY}{NOW}_txt{args.max_seq_len}_h{args.n_heads}_l{
            args.n_layers}_dff{args.ffn_dim}_d{args.dim}_bz{args.batch_size}"
    ...
    optimizer = torch.optim.AdamW(param_dicts, lr=args.lr, betas=(
        0.9, 0.98), eps=1e-09, weight_decay=args.weight_decay)
    dist.barrier()
    ...
    if os.path.exists(args.ckpt_dir):
        ...
        checkpoint = torch.load(
            args.ckpt_dir, map_location=lambda storage, loc: storage.cuda(gpu))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, batch in enumerate(tqdm.tqdm(train_dl, mininterval=10., desc='training')):
            ...
            seq = batch['seq'].to(gpu).long()
            # .cuda(args.gpu, non_blocking=True)
            # mask = batch['mask'].to(gpu).long()
            ################################################################################
            ...
            if gpu == 0 and args.profile is True:
                iter_time.append(time.monotonic() - batch_start)
                b_time.append(np.log2(time.monotonic() - backward))
                forward_top_stats = forward_trace2.compare_to(
                    forward_trace, 'lineno')

            if i == 30 and gpu == 0 and args.profile is True:
                ...
                print("\n")
                print("gb_in_use torch.cuda.max_memory_allocated",
                      round(gb_in_use, 3))
                print("Model Forward Timing (log_2(T)(sec)",
                      round(np.mean(f_time), 3))
                print("Model Backward Timing (log_2(T)(sec)",
                      round(np.mean(b_time), 3))
                print("\n\n")
            ...
            if i + 1 % args.validate_every == 0:
                ...
                if gpu == 0:
                    print(f'training loss: {
                          loss.item()} | validation loss: {val_loss.item()}')
            ...
            if i + 1 % args.generate_every == 0 and i != 0 and gpu == 0:
                ...
                if args.kernel_type == 'favor':
                    ...
                    raise "The kernel_type argument has to be one of followings; 'favor', 'generalised'"
        ...
        if not math.isfinite(loss):
            print(f'Evaluation loss is {
                  loss}, stopping training!!!!!!!!!!!!!!!!!!!!!!!!!!!')
            sys.exit(1)

    dist.destroy_process_group()
    print("\nThe whole ", epoch, " epoch took for",
          time.monotonic() - tot_start, "\n")


if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        parents=[parser_.parser], description="sut on performers")
