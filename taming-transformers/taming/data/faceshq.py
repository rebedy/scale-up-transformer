...
class FacesBase(Dataset):
    ...
    def __getitem__(self, i):
        # eg. example = { "image": ndarray(256, 256, 3) value -1.0 ~ 1.0  ,  "file_path_": 'data/ffhq/00141.png' }   from ImagePaths's __getitem__
        example = self.data[i]
        ex = {}
        if self.keys is not None:   # 보통 self.keys = None
            ...
            ex = example
        # 보통 { "image": ndarray(256, 256, 3) value -1.0 ~ 1.0  ,  "file_path_": 'data/ffhq/00141.png' }
        return ex
...
class FacesHQTrain(Dataset):
    ...
    def __init__(self, size, keys=None, crop_size=None, coord=False):
        ...
        # d2: <taming.data.faceshq.FFHQTrain object>
    ...
    def __getitem__(self, i):
        # self.data: <taming.data.base.ConcatDatasetWithIndex object>
        ex, y = self.data[i]
...
class FacesHQValidation(Dataset):
    ...
    def __getitem__(self, i):
        # self.data: <taming.data.base.ConcatDatasetWithIndex object>   ex = {image': array(256, 256, 3), 'file_path_': 'data/ffhq/00137.png'}  y = 0
        ex, y = self.data[i]
        if hasattr(self, "cropper"):  # True
            if not self.coord:  # self.coord = False
                # 이미 ex["image"]가 256x256으로 centercrop된 후에 나온건데 또 centercrop을 하네. 중복.
                out = self.cropper(image=ex["image"])
                ex["image"] = out["image"]
            else:
                h, w, _ = ex["image"].shape
