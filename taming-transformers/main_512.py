...
def get_parser(**parser_kwargs):  # parser_kwargs: {}
    def str2bool(v):
    ...
    parser.add_argument(
        "-b",
        "--base",
        nargs="*",   # 0개 이상의 값을 전부 읽어들인다. 리스트로 파싱됨.
        metavar="/configs/2021-10-10_512-project.yaml", #"base_config.yaml",
...
class DataModuleFromConfig(pl.LightningDataModule):
    def __init__(self, batch_size, train=None, validation=None, test=None,
        ...
        if test is not None:   # 보통 test는 None
    ...
    def setup(self, stage=None):  # stage 인자 쓸 일은 없지만 안 만들어주면 에러남
        self.datasets = dict(
            (k, instantiate_from_config(self.dataset_configs[k]))
            for k in self.dataset_configs)
        if self.wrap:  # 보통 False
...
class SetupCallback(Callback):
    ...
    def on_pretrain_routine_start(self, trainer, pl_module):  # Called at the beginning of the pretrain routine (between fit and train start).  Available Callback hooks
        if trainer.global_rank == 0:
            ...
            OmegaConf.save(self.config,   # type(self.config) = OmegaConf
...
class ImageLogger(Callback):
    def __init__(self, batch_frequency, max_images, clamp=True, increase_log_steps=True):
        super().__init__()
        self.batch_freq = batch_frequency  # eg. 750
        self.max_images = max_images       # eg. 4
