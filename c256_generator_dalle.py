...
#######!########
                #"/home/edlab/dylee/scaleup_transformer/transformer_c256/"
args.infer_ckpt= args.save_dir_base+"sut_c256_20211017_05h15m_totim1_im1_txt256_fixed_bz30_ep50_lr1e-05_relu/epoch=000050.ckpt"
...
if args.infer_profile:
    ...
    log_text = open(os.path.join(file_path, 'inference.log'), 'a')
    sys.stdout = log_text
...
args.img_fmap_size = int(_ds.img_fmap_size)  #16 image_seq_len
print("max_seq_len is : ", args.max_seq_len)
print("max_text_len is : ", args.max_text_len)
...
for epoch in range(args.start_epoch, args.start_epoch+1):
    ...
    for batch_idx, batch in enumerate(tqdm.tqdm(dl)):
        ...
        with torch.no_grad():
            ...
            #!# text generation
            b, h, w, c, device = *images.shape, images.device
            out = torch.tensor([[sos_token_idx]]*b).to(device)
            # print("images.shape", images.shape) images.shape torch.Size([32, 256, 256, 3])
            ...
            for cur_len in range(args.condition_len+1, args.max_seq_len):
                # image, text = out[:, :args.condition_len], out[:, args.condition_len:]
                logits = model(images, out) # -> [b, 64+256, num_text_tokens]
                logits = logits[:, -1, :]
                ...
                ## break check
                if ( (out[:, args.condition_len:] == eos_token_idx).sum(dim=-1) > 0 ).sum() == b:
                    break

            gen_out = out[:, args.condition_len:]  # [B, <text_seq_len]
            ## postprocess
