...
#!#  
""" 
CUDA_VISIBLE_DEVICES=6,7 python -m torch.distributed.launch --nproc_per_node=2 c256_main_dalle.py
"""
...
def train_one_epoch(gpu, model, train_loader, scaler, optimizer, epoch, args):
    ...
    for i, batch in enumerate(tqdm.tqdm(train_loader)):
        ...
        with autocast():
            ...
            # logit = logits[:, args.condition_len:].reshape(-1, logits.size(-1))
...
def main_worker(args: argparse.Namespace):
    ...
    if gpu == 0:
        if args.wandb:
            wandb.init(dir=args.save_dir_base, config = args, project="SUT_Transformers_DALLE_"+TODAY)
        # train_ds  81271
        # train_dl  6773
        # val_ds  678
        # val_dl  57
        ...
        # test_dl  109
        print("\n")        
        print(" >>> Transformer CNN 256 version!!!!!")
        ...
        ## Make DIRs
        exp_name = f"sut_c256_{TODAY}{NOW}_totim{args.target_count}_im{args.max_img_num}_txt{args.max_text_len}_{args.under_sample_type}_bz{args.batch_size}_ep{args.epochs}_lr{args.lr}_{args.activation}"
    ...
    tokenizer_.add_special_tokens(['[SOS]', '[EOS]', '[PAD]'])
