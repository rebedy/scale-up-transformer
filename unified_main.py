...
CUDA_VISIBLE_DEVICES=6 python3 unified_main.py --vqgan 512 --transformer --target_count 2 --max_img_num 2 --attn_type conditioned_noncuda --FAVOR --generalized_attention
CUDA_VISIBLE_DEVICES=4,5 python3 unified_main.py --vqgan 512 --transformer --target_count 2 --max_img_num 2 --attn_type conditioned_noncuda --FAVOR --generalized_attention
CUDA_VISIBLE_DEVICES=6,7 python3 unified_main.py --vqgan 512 --transformer --target_count 2 --max_img_num 2

CUDA_VISIBLE_DEVICES=5 python3 unified_main.py --vqgan 512 --transformer --FAVOR --generalized_attention --attn_type conditioned_cuda --target_count 2 --max_img_num 2
CUDA_VISIBLE_DEVICES=6,7 python3 unified_main.py --vqgan 512 --transformer --FAVOR --generalized_attention --attn_type conditioned_cuda --target_count 2 --max_img_num 2

--test
CUDA_VISIBLE_DEVICES=2,3 python3 unified_main.py --vqgan 512 --transformer --FAVOR --generalized_attention --attn_type conditioned_cuda --target_count 2 --max_img_num 2
CUDA_VISIBLE_DEVICES=4,5 python3 unified_main.py --vqgan 512 --transformer --attn_type conditioned_cuda --target_count 2 --max_img_num 2
CUDA_VISIBLE_DEVICES=6,7 python3 unified_main.py --vqgan 512 --transformer --FAVOR --attn_type conditioned_cuda --target_count 2 --max_img_num 2


CUDA_VISIBLE_DEVICES=2,3 python3 unified_main.py --vqgan 512 --transformer --FAVOR --generalized_attention --attn_type conditioned_cuda --target_count 2 --max_img_num 2 --test
...
if __name__ == '__main__':
    ...
    # dataset args
    parser.add_argument(
        '--train_meta_file', default='metadata/__preprocessed_mimiccxr_train.csv', type=str)
    parser.add_argument(
        '--val_meta_file', default='metadata/__preprocessed_mimiccxr_validate.csv', type=str)
    parser.add_argument(
        '--test_meta_file', default='metadata/__preprocessed_mimiccxr_test.csv', type=str)
    parser.add_argument(
        '--img_root_dir', default='/home/edlab/dylee/mimic/sut_image_preprocessing2/', type=str)
    parser.add_argument(
    ...
    # parser.add_argument('--reload_ckpt_dir', default="/home/edlab/dylee/scaleup_transformer/unified_Performers/unified_sut_conditioned_causal_2of2_20220412_22h06m_d256_l4_h4_conditioned_cuda_res512_trans_gen/epoch=33-step=39643.ckpt", type=str)
    ...
    if args.vqgan == 256:
        ...
        args.vqgan_model_path = '/home/edlab/dylee/last.ckpt'
        args.vqgan_config_path = '/home/edlab/dylee/2022-06-01T01-54-16-project.yaml'
        args.codebook_indices_path = '/home/edlab/dylee/mimiccxr_vqgan_preprocessed_codebook_indices.pickle'
    ...
    tokenizer.enable_padding(pad_id=tokenizer.token_to_id(
        "[PAD]"), pad_token="[PAD]", length=args.max_text_len)
    ...
    base_path = "/home/edlab/dylee/scaleup_transformer/unified_Performers/"
    folder_path = "unified_sut_" + str(args.causal_clm) + "_" + str(args.max_img_num) + "of" + \
        str(args.target_count) + "_" + TODAY + NOW + "_d" + str(args.dim) + "_l" + str(args.depth) + \
        "_h" + str(args.heads) + "_" + str(args.attn_type) + "_res" + str(args.vqgan)
    if args.transformer:
        if args.FAVOR:
    ...
    if args.reload_ckpt_dir:
        model = model.load_from_checkpoint(
            args.reload_ckpt_dir)  # , strict=False)

    # instrument experiment with W&B
    args.wandb_name = NOW
    wandb_logger = WandbLogger(entity='sut', project='unified_Performer_' + TODAY, name=args.wandb_name, log_model=True, config=args)
    ...
    if (args.fp16 is True and args.sharded_ddp is True):
        ...
        trainer = pl.Trainer(**trainer_args, logger=wandb_logger, plugins=DDPPlugin(find_unused_parameters=True),
                             gradient_clip_val=args.gradient_clip_val, profiler="simple", accumulate_grad_batches=args.accumulate_grad_batches)
