...
def greedy_search(model,
    ...
    """
    with torch.no_grad():
        probabilities = torch.zeros(X.shape[0]).to(
...
def sample(model,
    ...
    with torch.no_grad():
        ...
        for i in iterator:
            ...
            random = torch.rand((next_probabilities.shape[0], 1)).to(
                next(model.parameters()).device)
            next_chars = ((next_probabilities.cumsum(1) < random).sum(1, keepdims=True))
            probabilities += torch.gather(input=next_probabilities.log(),
                                          dim=1, index=next_chars).squeeze()
            X = torch.cat((X, next_chars), axis=1)
        return X, probabilities
    ...
    with torch.no_grad():
        ...
        for i in predictions_iterator:
            ...
            best_candidates += torch.arange(
                X.shape[0] // beam_width, device=X.device).unsqueeze(-1) * beam_width
